...
int main()
{
	...
	while (1) {
		...
		// move our player here using wasd
		...
		// for each column on a screen calculate what to show
		for (int x = 0; x < nScreenWidth; ++x) {
			// For each column, calculate the projected ray angle into world space
			float fRayAngle = (fPlayerA - fFOV / 2.0f) + ((float)x / (float)nScreenWidth) * fFOV;
			float fStepSize = 0.1f;		   // Increment size for ray casting, decrease to increase resolution	
			float fDistanceToWall = 0.0f;  // Расстояние до препятствия в направлении fRayAngle for one of ray
			float fEyeX = sinf(fRayAngle); // Unit vector for ray in player space
			float fEyeY = cosf(fRayAngle);

			bool bHitWall = false;		   // Достигнул ли луч стенку
			bool bBoundary = false;		   // edge of a wall 
			// calculate how faw (fDisanceWall) and if we hit a wall or infinity
			...
			// we draw not only walls but also ceiling and floor and for 3D illusion we need to understand how 
			// far they are. if an object is close than it's big otherwise it will be small and the floor and the 
			// ceiling will be bigger
